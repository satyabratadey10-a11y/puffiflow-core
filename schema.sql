...
-- Trigger to auto-create profile on new confirmed user registration in auth.users
CREATE OR REPLACE FUNCTION public.handle_new_user()
RETURNS TRIGGER AS $$
BEGIN
  -- Only populate public.profiles if the user's email has been confirmed
...
-- RPC Function: Narrow email existence check for confirmed accounts only
...
-- Cleanup unverified unconfirmed abandoned accounts from auth.users
