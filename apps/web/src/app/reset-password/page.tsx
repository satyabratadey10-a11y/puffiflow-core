      ...
      <div className="w-full max-w-md p-8 rounded-3xl bg-slate-900/90 border border-slate-800 backdrop-blur-xl shadow-2xl space-y-6">
        <div className="text-center space-y-2">
          ...
          <p className="text-slate-400 text-xs md:text-sm">
            Enter the 6-digit recovery code sent to <span className="text-white font-medium">{email || 'your email'}</span>
          </p>
        </div>
        ...
        <form onSubmit={handleResetSubmit} className="space-y-4">
            ...
            <div className="flex justify-between gap-2" onPaste={handlePaste}>
              {otp.map((digit, idx) => (
                <input
                  ...
                  className="w-12 h-12 text-center text-lg font-mono font-bold rounded-xl bg-slate-950 border border-slate-800 text-white focus:outline-none focus:border-cyan-500 transition-colors"
                />
              ))}
            </div>
          </div>
          ...
          <button
            ...
            className="w-full py-3.5 px-4 rounded-xl font-bold text-sm text-white bg-gradient-to-r from-cyan-600 via-indigo-600 to-violet-600 hover:from-cyan-500 hover:to-violet-500 shadow-lg shadow-cyan-600/25 transition-all flex items-center justify-center space-x-2 disabled:opacity-50 mt-4"
          >
